...
function validateTaskPayload(taskData, { partial = false } = {}) {
  ...
  if (typeof payload.taskDate !== "undefined") {
    if (!isValidDateOnly(payload.taskDate)) {
...
class TaskService {
  async createTask(taskData) {
    const payload = validateTaskPayload(taskData);
  ...
  async updateTask(id, taskData) {
    ...
    const payload = validateTaskPayload(taskData, { partial: true });
